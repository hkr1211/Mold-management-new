# tests/test_mold_management_entrypoint.py — 模具管理页面入口回归测试
...
def _load_page(st_mock, has_permission=None):
    ...
    database.get_mold_statuses = lambda *a, **kw: []
    ...
    auth.has_permission = has_permission or (lambda *a, **kw: True)
    ...
    page_path = os.path.join(
        os.path.dirname(__file__), "..", "app", "pages", "1_模具管理.py"
    )
    with open(page_path, encoding="utf-8") as f:
        src = f.read()

    module = types.ModuleType("mold_page")
...
def test_blocks_unauthorized_role():
    """已登录但无 view_molds 权限时应被权限错误 + st.stop 拦截。"""
    st = _build_st_mock({"logged_in": True, "user_role": "访客"})
    with pytest.raises(SystemExit):
        _load_page(st, has_permission=lambda *a, **kw: False)
...
def test_authorized_load_with_empty_data_does_not_crash():
    ...
    _load_page(st)  # 顶层执行三个 tab，不应抛异常
    # 空数据列表应提示而非报错
    assert all("加载失败" not in msg for msg in st._error_calls)
