# tests/test_schedule_entrypoint.py — 生产排程页面入口回归测试
...
def _load_page(st_mock, has_perm=lambda perm: True):
    ...
    # 其它测试（如 *_schema_compat）可能在 sys.modules 中留下 plotly 的 mock 占位，
    # 这里清除无 __file__ 的占位后强制真实导入，确保 go.Scatter / px.* 可用。
    ...
    page_path = os.path.join(
        os.path.dirname(__file__), "..", "app", "pages", "8_生产排程.py"
    )
...
def test_authorized_load_with_empty_data_does_not_crash():
    ...
    _load_page(st)  # 顶层 show()：4 个 tab 在空数据下渲染不应抛异常
